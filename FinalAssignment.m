...
for t = 0:dt:T_end
    ...
    % 湍流扩散位移
    dx = u*dt + sqrt(2*Kx*dt)*randn(numActive, 1);
    dy = v*dt + sqrt(2*Ky*dt)*randn(numActive, 1);
    dz = w*dt + sqrt(2*Kz*dt)*randn(numActive, 1);
    
    % 更新粒子位置
    particles(active_idx, 1) = particles(active_idx, 1) + dx;
    particles(active_idx, 2) = particles(active_idx, 2) + dy;
    particles(active_idx, 3) = particles(active_idx, 3) + dz;
